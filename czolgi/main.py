...
def rysuj(p1, p2, blokady, komunikat=""):
    ...
    print()
    print("Legenda: P = Gracz 1, E = Gracz 2, # = blokada, . = teren\n")
...
class Czolag:
    ...
    def rusz(self, kierunek, blokady, pos_drugiego):
        if self.ruchy <= 0:
            ...
            return False
        nx, ny = self.x, self.y
        if kierunek in ('w','i'): ny -= 1; opis = "północ"
        elif kierunek in ('s','k'): ny += 1; opis = "południe"
        elif kierunek in ('a','j'): nx -= 1; opis = "zachód"
        elif kierunek in ('d','l'): nx += 1; opis = "wschód"
        else: return False
...
if __name__ == "__main__":
    ...
    while p1.zyje() and p2.zyje():
        ...
        while tura_trwa and aktywny.ruchy > 0 and p1.zyje() and p2.zyje():
            ...
            if aktywny is p1:
                print("w/a/s/d = ruch, f [ap|he] = STRZAŁ (po nim 1 krok i KONIEC tury), koniec = koniec tury, q = wyjście")
            else:
                print("i/j/k/l = ruch, h [ap|he] = STRZAŁ (po nim 1 krok i KONIEC tury), koniec = koniec tury, q = wyjście")

            wej = input("> ").strip().lower().split()
            if not wej: continue
            cmd = wej[0]

            if cmd == 'q': print("Koniec gry."); exit(0)
            if cmd == 'koniec': break
            ...
            if (aktywny is p1 and cmd == 'f') or (aktywny is p2 and cmd == 'h'):
                ...
                kier = input("Krok po strzale (w/a/s/d lub i/j/k/l). Enter = pomiń: ").strip().lower()
        ...
        input("\n--- Koniec tury. Naciśnij Enter, aby kontynuować ---")
